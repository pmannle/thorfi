...
import { Web3ReactProvider, useWeb3React } from './XdefiReactProvider';
import {
  WalletsModal,
  useWalletEvents,
  DisconnectBtn,
...
export const XdefiWalletContext = createContext<XdefiWallet | undefined>(undefined);

export function XdefiWalletProvider({ children }: UIElementProps) {
  return (
    <Web3ReactProvider>
...
function WalletProvider({ children }: UIElementProps) {
  ...
  const evmWallet = useMemo<XdefiWallet>(() => {
  ...
  return (
    <XdefiWalletContext.Provider value={evmWallet}>
      {children}
